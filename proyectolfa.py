...
class BibliotecaDigital:
    ...
    def analizar_linea_usuario(objeto, linea, numero_linea):
        ...
        # Verificar que tenga exactamente 2 partes (ID y nombre)
        if len(partes) != 2:
            print(f"ERROR - Línea {numero_linea}: Formato incorrecto, se esperan 2 campos separados por '{separador}' el ID y el Nombre)")
            return None
        
        try:
            # ID Usuario (debe ser numérico)
    ...
    def analizar_linea_libro(objeto, linea, numero_linea):
        ...
        # Verificar que tenga exactamente 2 partes (ID y título)
        if len(partes) != 2:
    ...
    def analizar_linea_prestamo(objeto, linea, numero_linea):
        ...
        #id_usuario,nombre_usuario,id_libro,titulo_libro,fecha_prestamo,(fecha_devolucion-P.VACIO)
        ...
        # Verificar que tenga al menos 5 partes (fecha_devolucion puede estar vacía)
        if len(partes) < 5:
    ...
    def mostrar_historial_prestamos(objeto):
        ...
        print(f"{'ID Usuario':^6} {'Nombre Usuario':^6} {'ID Libro':^6} {'Título Libro':^6} {'Fecha Préstamo':^6} {'Fecha Devolución':^6}")
        print("-" * 120)
        
        for prestamo in objeto.prestamos:
            devolucion = prestamo.fecha_devolucion if prestamo.fecha_devolucion else "Pendiente"
            print(f"{prestamo.id_usuario:<^6} {prestamo.nombre_usuario:^6} {prestamo.id_libro:^6} {prestamo.titulo_libro:^6} {prestamo.fecha_prestamo:^6} {devolucion:^6}")
    ...
    def mostrar_usuarios_unicos(objeto):
        ...
        print(f"{'ID Usuario':^6} {'Nombre Usuario':^6}")
        print("-" * 45)
        
        for id_usuario, nombre in usuarios_unicos.items():
            print(f"{id_usuario:^6} {nombre:^6}")
    ...
    def mostrar_libros_prestados(objeto):
        ...
        print(f"{'ID Libro':^6} {'Título Libro':^6}")
        print("-" * 50)
        
        for id_libro, titulo in libros_unicos.items():
            print(f"{id_libro:^6} {titulo:^6}")
    ...
    def mostrar_estadisticas_prestamos(objeto):
        ...
        print(f"{'Estadística':^6} {'Valor':^6}")
        print("-" * 50)
        print(f"{'Total de préstamos:':^6} {total_prestamos:^6}")
        print(f"{'Libro más prestado:':^6} {libro_mas_prestado} ({max_prestamos} veces)")
        print(f"{'Usuario más activo:':^6} {usuario_mas_activo} ({max_prestamos_usuario} préstamos)")
        print(f"{'Total usuarios únicos:':^6} {usuarios_unicos:^6}")
    ...
    def mostrar_prestamos_vencidos(objeto):
        ...
        for prestamo in objeto.prestamos:
            ...
            if not prestamo.fecha_devolucion:
                # Verificar si la fecha de préstamo + tiempo razonable ya pasó
                try:
                    fecha_prestamo = datetime.strptime(prestamo.fecha_prestamo, '%Y-%m-%d').date()
                    # Asumimos 30 días como tiempo límite de préstamo
                ...
                # Verificar si ya pasó la fecha de devolución y no se ha devuelto
                try:
        ...
        print(f"{'ID Usuario':^6} {'Nombre Usuario':^6} {'ID Libro':^6} {'Título Libro':^6} {'Fecha Préstamo':^6} {'Fecha Límite':^6}")
        print("-" * 120)
        
        for prestamo in prestamos_vencidos:
            fecha_limite = prestamo.fecha_devolucion if prestamo.fecha_devolucion else "30 días desde préstamo"
            print(f"{prestamo.id_usuario:^6} {prestamo.nombre_usuario:^6} {prestamo.id_libro:^6} {prestamo.titulo_libro:^6} {prestamo.fecha_prestamo:^6} {fecha_limite:^6}")
